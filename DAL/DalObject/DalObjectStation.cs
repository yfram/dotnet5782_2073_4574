...
namespace DalObject
{
    public partial class DalObject
    {
        public void AddStation(int id, string name, double longitude, double lattitude, int chargeSlots) =>
            DataSource.Stations.Add(new(GetStationIndex(id) != -1 ? throw new Exception($"the Station {id} is already exsist!") : id, name, longitude, lattitude, chargeSlots));
        public string GetStationString(int id)
        {
            int ix = GetStationIndex(id);
            if (ix == -1)
                throw new ArgumentException($"the Station {id} is not exsist!");
            return DataSource.Stations[ix].ToString();
        }
